...
def construct_experiment(solver_path, map_path, scen_path, agent_num, seed, neighborhood_size, experiment_name, destroy_strategy):
    ...
    return [solver_path, '-m', map_path, '-a', scen_path, "-k", str(agent_num), "--initAlgo", "PP", "--maxIterations",  str(max_iter), "--initLNS", "0", "--seed", str(seed), "-t", str(time_limit), "--neighborSize", str(neighborhood_size), "--destoryStrategy", destroy_strategy, "--outputPaths", "./repair_sols/" + experiment_name, "--restarts", "1"]
...
num_runs = 50
results_file = "results_repair_initsol"
