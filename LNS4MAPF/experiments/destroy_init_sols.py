...
def construct_experiment(solver_path, map_path, scen_path, agent_num, seed, neighborhood_size, experiment_name, destroy_strategy):
    """ Construct the input of subprocess.check_output to perform the experiment"""
    return [solver_path, '-m', map_path, '-a', scen_path, "-k", str(agent_num), "--initAlgo", "PP", "--maxIterations",  str(max_iter), "--initLNS", "0", "--seed", str(seed), "-t", str(time_limit), "--neighborSize", str(neighborhood_size), "--destoryStrategy", destroy_strategy, "--outputPaths", "./dest_sols/" + experiment_name, "--restarts", "1"]
...
num_runs = 50
results_file = "results_destroy_initsol"
...
for map_name in tqdm.tqdm(map_names, desc="maps", position=0):
    ...
    for agent_num in tqdm.tqdm(agent_nums[map_idx], desc="agent nums", position=1, leave=False):
        for i in tqdm.tqdm(range(num_runs), desc="runs", position=2, leave=False):
            ...
            # find the seed
            experiment_name = "destroy_" + str(agent_num) + "agents_" + map_name + "_" + scen_name_stripped +  "_SIPP_mapf_lns_destroy_type_ADAPTIVE_neighborhood_size_8"+ "_" + str(int(i/len(scenes))) + ".json"
            ...
            experiment_name = "destroy_" + str(agent_num) + "agents_" + map_name + "_" + scen_name_stripped + "_" + str(int(i/len(scenes)))
            exp = construct_experiment(solver_path2, map_path, scen_path, agent_num, seed, 8, experiment_name, "Adaptive")
            try:
                res = subprocess.check_output(exp)[:-1].decode('UTF-8')
            except:
                print("Solver returned error for map: ", map_name, ", scene: ", scen_name_stripped)
