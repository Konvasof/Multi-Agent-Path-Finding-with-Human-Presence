...
def construct_experiment(solver_path, map_path, scen_path, output_file_name, agent_num, seed, neighborhood_size, experiment_name, destroy_strategy, time_limit):
    ...
    return [solver_path, '-m', map_path, '-a', scen_path, "-o", output_file_name + ".csv", "-k", str(agent_num), "--initAlgo", "PP", "--maxIterations",  str(max_iter), "--initLNS", "0", "--seed", str(seed), "-t", str(time_limit), "--stats", "./repair_exp/" + experiment_name, "--neighborSize", str(neighborhood_size), "--destoryStrategy", destroy_strategy, "--restarts", "1"]
    ...
    for neighborhood_size in neighborhood_sizes:
        ...
        for agent_num in tqdm.tqdm(agent_nums[map_idx], desc="agent nums", position=1, leave=False):
            for i in tqdm.tqdm(range(num_runs), desc="runs", position=2, leave=False):
                ...
                # find the seed
                experiment_name = "repair_" + str(agent_num) + "agents_" + map_name + "_" + scen_name_stripped +  "_SIPP_mapf_lns_" + str(int(i/len(scenes))) + ".json"
                df = pd.read_sql_query(f"SELECT * FROM repair WHERE file_name = '{experiment_name}'", conn)
                seeds = df["seed"]
                if (len(seeds) != 1):
                ...
                for strategy in destroy_strategies:
                    experiment_name = "repair_" + str(agent_num) + "agents_" + map_name + "_" + scen_name_stripped + "_" + str(int(i/len(scenes)))
